...
const vimeo = `<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Vimeo</title><path d="M23.9765 6.4168c-.105 2.338-1.739 5.5429-4.894 9.6088-3.2679 4.247-6.0258 6.3699-8.2898 6.3699-1.409 0-2.578-1.294-3.553-3.881l-1.9179-7.1138c-.719-2.584-1.488-3.878-2.312-3.878-.179 0-.806.378-1.8809 1.132l-1.129-1.457a315.06 315.06 0 003.501-3.1279c1.579-1.368 2.765-2.085 3.5539-2.159 1.867-.18 3.016 1.1 3.447 3.838.465 2.953.789 4.789.971 5.5069.5389 2.45 1.1309 3.674 1.7759 3.674.502 0 1.256-.796 2.265-2.385 1.004-1.589 1.54-2.797 1.612-3.628.144-1.371-.395-2.061-1.614-2.061-.574 0-1.167.121-1.777.391 1.186-3.8679 3.434-5.7568 6.7619-5.6368 2.4729.06 3.6279 1.664 3.4929 4.7969z"/></svg>`;
...
const pinterest = `<svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Pinterest</title><path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.162-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.957 1.406-5.957s-.359-.72-.359-1.781c0-1.663.967-2.911 2.168-2.911 1.024 0 1.518.769 1.518 1.688 0 1.029-.653 2.567-.992 3.992-.285 1.193.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.063-4.869-5.008-4.869-3.41 0-5.409 2.562-5.409 5.199 0 1.033.394 2.143.889 2.741.099.12.112.225.085.345-.09.375-.293 1.199-.334 1.363-.053.225-.172.271-.401.165-1.495-.69-2.433-2.878-2.433-4.646 0-3.776 2.748-7.252 7.92-7.252 4.158 0 7.392 2.967 7.392 6.923 0 4.135-2.607 7.462-6.233 7.462-1.214 0-2.354-.629-2.758-1.379l-.749 2.848c-.269 1.045-1.004 2.352-1.498 3.146 1.123.345 2.306.535 3.55.535 6.607 0 11.985-5.365 11.985-11.987C23.97 5.39 18.592.026 11.985.026L12.017 0z"/></svg>`;
...
const socialIcons = [[facebook, '#1877F2'], [twitter, '#1DA1F2'], [instagram, '#E4405F'], [linkedin, '#0A66C2'], [tiktok, '#000000'], [pinterest, '#BD081C'], [youtube, '#FF0000'], [vimeo, '#1AB7EA']];
function socials() {
  const socialContainer = document.querySelectorAll('.social__container');
  const socialMediaSection = document.querySelector('.social-media');
  socialContainer.forEach(el => {
    const anchor = el.querySelector('a');
    const link = anchor.innerHTML;
    swapHTML(anchor, link, el);
    socialMediaSection.querySelector('.bp-profile__section--content').style.display = 'flex';
  });
}

function swapHTML(anchor, link, el) {
  if (anchor.innerText.includes('facebook')) {
    el.innerHTML = '';
    anchor.innerHTML = `<a href="${link}" target="_blank" class="social--link" >${socialIcons[0][0]}</a>`;
    el.innerHTML = anchor.innerHTML;
    const svg = el.querySelector('svg');
    svg.style.fill = `${socialIcons[0][1]}`;
  }

  if (anchor.innerText.includes('twitter')) {
    el.innerHTML = '';
    anchor.innerHTML = `<a href="${link}" target="_blank" class="social--link" >${socialIcons[1][0]}</a>`;
    el.innerHTML = anchor.innerHTML;
    const svg = el.querySelector('svg');
    svg.style.fill = `${socialIcons[1][1]}`;
  }

  if (anchor.innerText.includes('instagram')) {
    el.innerHTML = '';
    anchor.innerHTML = `<a href="${link}" target="_blank" class="social--link" >${socialIcons[2][0]}</a>`;
    el.innerHTML = anchor.innerHTML;
    const svg = el.querySelector('svg');
    svg.style.fill = `${socialIcons[2][1]}`;
  }

  if (anchor.innerText.includes('linkedin')) {
    el.innerHTML = '';
    anchor.innerHTML = `<a href="${link}" target="_blank" class="social--link" >${socialIcons[3][0]}</a>`;
    el.innerHTML = anchor.innerHTML;
    const svg = el.querySelector('svg');
    svg.style.fill = `${socialIcons[3][1]}`;
  }

  if (anchor.innerText.includes('tiktok')) {
    el.innerHTML = '';
    anchor.innerHTML = `<a href="${link}" target="_blank" class="social--link" >${socialIcons[4][0]}</a>`;
    el.innerHTML = anchor.innerHTML;
  }

  if (anchor.innerText.includes('pinterest')) {
    el.innerHTML = '';
    anchor.innerHTML = `<a href="${link}" target="_blank" class="social--link" >${socialIcons[5][0]}</a>`;
    el.innerHTML = anchor.innerHTML;
    const svg = el.querySelector('svg');
    svg.style.fill = `${socialIcons[5][1]}`;
  }

  if (anchor.innerText.includes('youtube')) {
    el.innerHTML = '';
    anchor.innerHTML = `<a href="${link}" target="_blank" class="social--link" >${socialIcons[6][0]}</a>`;
    el.innerHTML = anchor.innerHTML;
    const svg = el.querySelector('svg');
    svg.style.fill = `${socialIcons[6][1]}`;
  }

  if (anchor.innerText.includes('vimeo')) {
    el.innerHTML = '';
    anchor.innerHTML = `<a href="${link}" target="_blank" class="social--link" >${socialIcons[7][0]}</a>`;
    el.innerHTML = anchor.innerHTML;
    const svg = el.querySelector('svg');
    svg.style.fill = `${socialIcons[7][1]}`;
  }
}
...
/* harmony import */ var _modules_profileScripts__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./modules/profileScripts */ "./src/modules/profileScripts.js");
...
/* harmony import */ var _modules_loggedOutHeader__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./modules/loggedOutHeader */ "./src/modules/loggedOutHeader.js");
/* harmony import */ var _modules_membershipAsterisk__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./modules/membershipAsterisk */ "./src/modules/membershipAsterisk.js");
...
function init() {
  (0,_modules_copyright__WEBPACK_IMPORTED_MODULE_2__.sparkCopyrightInjection)();
  (0,_modules_loggedOutHeader__WEBPACK_IMPORTED_MODULE_4__["default"])();
  let URL = window.location.href; // sparkFeaturedJob();

  if (URL.includes('checkout')) {
    window.addEventListener('load', _modules_membershipAsterisk__WEBPACK_IMPORTED_MODULE_5__.controlAsterisk);
  }

  if (URL.includes('profile')) {
    (0,_modules_profileScripts__WEBPACK_IMPORTED_MODULE_1__["default"])();
  }

	...
	<?php foreach ($application_fields as $key => $field) : ?>
		...
		<?php else : ?>
			<fieldset class="fieldset-<?php echo esc_attr($key); ?>">
				<label for="<?php echo esc_attr($key); ?>"><?php echo wp_unslash($field['label']) . apply_filters('submit_job_form_required_label', $field['required'] ? '' : ' <small>' . __('(Recommended)', 'wp-job-manager-applications') . '</small>', $field); ?></label>

    ...
    if (resume_manager_user_can_post_resume()) :
        ...
        foreach ($resume_fields as $key => $field) : ?>
            <fieldset class="fieldset-<?php esc_attr_e($key); ?>">
                <label for="<?php esc_attr_e($key); ?>"><?php echo $field['label'] . apply_filters('submit_resume_form_required_label', $field['required'] ? ' <img class="requiredAsterisk" src=' . wp_get_attachment_url(13) . '>' : ' <small>' . __('(Recommended)', 'wp-job-manager-resumes') . '</small>', $field); ?></label>
                <div class="field">
